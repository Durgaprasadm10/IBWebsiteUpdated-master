...
$objPHPExcel->getActiveSheet()->setCellValue('A1', 'PageName');
...
if ($count > 0) {
    ...
    foreach ($analyticsData as $aData) {
        $objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $aData['page_name']);
...
header('Content-Disposition: attachment;filename="Vistortrackingreport_byPage-' . date("Y-m-d-H-i-s") . '.xls"');
