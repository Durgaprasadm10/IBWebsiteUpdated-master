 ...
 * Description : this page will produce Excel that                		 *
                 contains given country analytics by the page    		 *
				 country_analytics.php                          		 *
 ************************************************************************/
...
$country_name_input = $_POST['Countryhidden'];

$sql = "SELECT vi.*,p.page_name FROM ".VISITORS_INFO." as vi LEFT JOIN ".PAGE." as p ON p.id=vi.page_id WHERE vi.geo_info_status = 1 ".$deviceCond." AND vi.country = '".$country_name_input."'".$friendlyIpsCond.$friendlyWebsites;
...
header('Content-Disposition: attachment;filename="Country-' . date("Y-m-d-H-i-s") . '.xls"');
