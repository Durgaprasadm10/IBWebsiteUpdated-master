...
$sStartDate = date('Y-m-d', strtotime('first day of previous month'));
$sEndDate = date('Y-m-d', strtotime("-1 days"));

foreach ($objCList->getUniqIpList($sStartDate, $sEndDate) as $data) {
    array_push($aHitCount2, $objCList->gethitCountByIp($data['ip_address'], $sStartDate, $sEndDate));
...
$url = TRACKING_URL . '/visitors_by_ip_ns.php';
?>
            ...
            <table cellpadding="5" style="border:1px solid black;border-collapse:collapse;margin-left:40px;">
                ...
                if (sizeof($aIpList) > 0) {
                    ...
                    if ($iTables > 0) {
                        ...
                        echo "<th style='border:1px solid black;'>Ip Address</th><th style='border:1px solid black;'>Hit Count</th>";
                        echo "<tr><td style='border:1px solid black;text-align:center;' colspan='2'>No record found</td></tr>";
                    }
                    echo "</thead>";
                    $r = 1;
                    $i = 0;
