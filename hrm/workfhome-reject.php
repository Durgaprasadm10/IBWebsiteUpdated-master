...
echo $reason;
exit();*/
$updat = mysql_query("UPDATE wp_workf_home SET status = '$reason Rejected' WHERE `ecode` = '$use' AND `from`= '$fro' AND `to` = '$to'  AND `status` = 'pending' ");
...
echo 'alert("Rejected.");'; 
echo 'window.location.href = "http://hrm.ideabyte.net/work-from-home-status/"';
echo '</script>';
}
?>
