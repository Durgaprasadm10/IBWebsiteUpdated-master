<?php /* Template Name: APPLY COMPOFF */ ?>
<?php
   global $wpdb;
get_header();
ob_start();
...
$checkdate = ("SELECT *FROM wp_leave_compoff WHERE ecode = '$rid'"); // AND from = '$sdat'
...
$wpdb->insert('wp_leave_compoff', array(
    'ecode' => $rid,
'name' => $stelaemp,
    'mgrid' => $user_last,
    'from' => $sdat, 
...
//echo $to;
//exit;
$approve = 'http://ideabytes.com/hrm/compoff-activate.php?ecode='.$rid.'&&from='.$sdat.'&&to='.$tda;
$reject = 'http://ideabytes.com/hrm/compoff-reject.php?ecode='.$rid.'&&from='.$sdat.'&&to='.$tda;
...
echo '<h2>Your leave request successfully sent to your manager. Please wait for Compoff approval.</h2>';
//echo $mgri;
//echo $usermail;
}
