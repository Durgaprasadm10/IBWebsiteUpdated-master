...
class PDb_Init {
  ...
  private function _uninstall()
  {
    ...
    // remove options
    $option_list = array(
        Participants_Db::$participants_db_options,
        Participants_Db::$db_version_option,
        Participants_Db::$default_options,
        Participants_Db::$prefix . 'csv_import_params',
        Participants_Db::$prefix.'option_version',
    );
    ...
    $transient_list = array(
        Participants_Db::$last_record,
    );
    // find all plugin transients and add them to the list to be deleted
    $sql = 'SELECT REPLACE(`option_name`, "_transient_", "") FROM ' . $wpdb->options . ' WHERE `option_name` LIKE "_transient_' . Participants_Db::$prefix . '%"';
    $transient_list = array_merge( $transient_list, $wpdb->get_col( $sql ) );
